...
out=$(sudo faramir secrets edit -editor /bin/true "$(basename "$store_file")" 2>&1)
case "$out" in
  *unchanged*) ok "sudo faramir secrets edit still opens the store" ;;
  *) die "faramir secrets edit failed without the key: $out" ;;
esac
