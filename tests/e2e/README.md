...
`SUDO` unset is the original sudo and the names this harness has always used, so a command that does not ask for an arrangement behaves as it did.
...
| `check-refuse.sh` | `[[secret.refuse]]`: a path refused to the agent's file tools, and the two costs of never reading it: the mode is left alone and the value is absent from the redactor |
