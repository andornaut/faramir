...
# Claude Code checks the paths a Bash command names against its deny list before
# it acts on the hook's decision: a Read rule covering the wrapper refuses every
# Bash call in an enrolled tree, and an Edit rule makes every one ask. Checked
# against the rendered file rather than the renderer: this is where a layout the
# renderer and the guard spell differently would show.
CLAUDE_RULES=/home/op/.claude/settings.json
if [ -f $CLAUDE_RULES ]; then
  grep -qF '(//usr/local/libexec/faramir' $CLAUDE_RULES \
    && bad "a rule names the wrapper's directory, which refuses or questions every Bash call in an enrolled tree" \
    || ok "no rule names the directory the wrapper is sourced from"
else
