...
- Every brokered command is confined to its own cgroup and reaped there, and the tests exercise that path: they need cgroup v2 with `cgroup.kill` (kernel 5.14 or newer) and a cgroup the test process can subdivide. Where that is not writable, an unprivileged CI runner in a root-owned service cgroup, hand the process a delegated one first, as [the test workflow](.github/workflows/test.yml) does. The `make test` targets do it for you, running the suite under `systemd-run --user --scope`, which inherits the delegation systemd gives `user@.service` and so asks for no privilege. Without it roughly sixty tests skip and the run still prints `ok`, so each target ends by naming what it did not check. Older kernels and cgroup v1 are unsupported.
