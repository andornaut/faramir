...
A skipped check is one warn line whatever it stood for, so a line under the totals counts them: without sudo the twelve boundary checks are a single finding, and the totals alone would read the same on a host examined in full and on one where fourteen questions were never put.

The two probes that run a brokered command, the SSH agent and the brokered command check, are skipped in the two states where the command cannot be sent: against a broker known to hold no values, which refuses them, the refusal being about the value set rather than about what was probed; and against a broker that answered nothing when the install was looked up, no brokered command reaching one that is not there. Neither is reported as a fault of what was probed, the outage being the socket and version checks' to report.

They part company on a broker that was never asked. The brokered command check needs root, so an unestablished value set there means `--check` itself did not report, and that is what it says. The SSH agent probe runs as the caller's own account, so it is answered without sudo, and a refusal that does come back is recognised as one. A refusal from a broker whose `--check` read every managed file fails rather than skips: the two disagree, which is a daemon that came up before the values were written and has not read them since.
