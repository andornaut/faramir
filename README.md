...
For the secrets it is one rule, and `exec` is held to it because a brokered command's output is redacted against the same set: **the broker serves `exec` and `redact` only while no managed file went unread.** At least one entry matched a file, and every matched file loaded. What those files held does not enter into it: a file that is there and empty has nothing to miss, so an install whose operator has not written a secret yet is configured correctly and serves. A ref no file defines is answered by `unknown_secret`, which says the true thing. Otherwise it refuses with `no_secrets`, naming why. It comes up either way and `status` and `list_secrets` keep answering, because those are what explain a missing store and neither produces output that depends on the value set. Per request, so a reload that shrinks the set later is caught too, which a startup gate never was. The exception is a keeper that could not be reached: the set kept then is the last one known to be true, unconfirmed rather than short, so a keeper hiccup does not become refused commands.
