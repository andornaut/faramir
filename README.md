...
>
> **If you already run the project with permissions bypassed, this costs you nothing.** Under `--dangerously-skip-permissions` Bash never prompted in the first place, so approving it here takes nothing away, while the deny list and the redactor both still apply: hook decisions are honoured in that mode, a hook `deny` included. Enrolling an unattended project is purely additive. Note that this is specifically the bypass mode. Auto-accepting **edits** (`acceptEdits`) leaves Bash prompting normally, so enrolling costs exactly what it costs in the default mode. See [What this gives up](docs/scope.md).
...
And an unattended run with `--dangerously-skip-permissions` can rewrite any repository you can. Hooks are not skipped in that mode and their decisions are honoured, a `deny` included, so both redaction and the deny list hold there exactly as they do anywhere else. Bounding what such a run may *change* is a matter of `gh` token scope and branch protection, not of this project.
