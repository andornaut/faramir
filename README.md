...
**A re-run keeps what the install already uses.** `config.toml` is rendered from this run's values and a drop-in may not set the install-owned ones, so a flag left out is taken from the install being re-provisioned rather than from the compiled-in default: the service accounts from each unit's `User=`, the client group and the SSH key from the installed config, the secrets group from the directory it owns. Each has one source, the one `doctor` reads. What was adopted is reported as its own step before anything is written with it, and only where it differs from the default, a re-run of an install that named none having nothing to say. A flag still outranks it, that being how any of them is changed. A first install has none of this to read. A `config.toml` that is there and will not parse stops the run before anything is written, whatever flags it was given: no daemon can load that file either, so the host is serving nothing, and re-provisioning over it replaces what says why. The file is yours to fix, or to remove for a fresh install.
...
`--config-dir DIR` | [found the usual way](#checking-an-install) | An absolute path for `config.toml`, `config.d/`, the age key and the managed sops files. Left out, it re-provisions the install this host already has, so a re-run need not repeat where that went. Naming one provisions that directory, which on a host whose install is elsewhere is a second install and not a move: `init` writes and never migrates, so the first one's keys, secrets and units stay where they are while the units are repointed at the new one. It has to be mounted before the daemons read it, and its parent has to exist: this is the one directory faramir creates whose parent can be yours, and creating one would hand it to root.
...
`--known-hosts PATH` | none | A `known_hosts` file whose host keys are pinned for the executor, copied to `<exec-home>/.ssh/known_hosts`. Opt-in: without it a brokered `ssh` verifies against `/etc/ssh/ssh_known_hosts` alone. A copy rather than a reference, the executor being unable to read your `0700 ~/.ssh`, and safe to copy where an ssh config is not, public host keys carrying no directive that executes anything. Replaced whole on each run rather than merged, `HashKnownHosts` leaving entries unmatchable by name and a blind append keeping a rotated host's old key as a second line ssh still accepts, so the file you name is the authority. A file that is not a `known_hosts` file is refused before anything is written, which is what catches a path that reaches a private key.
...
Most of the examination needs another uid: the broker's own `--check`, the comparison of the `.sops.yaml` recipients against the keeper's `0400` age key, and the dozen checks that ask what each account can reach. Each is asked as the account it is about, root bypassing file modes so the same question from root answers itself. Without sudo these report as unchecked rather than as passing, and the findings are ordered so they group at the end.
...
Two checks run a brokered command rather than reading a mode: the SSH agent probe and the brokered command check. Both are skipped where there is no answer to be had, against a broker known to hold no values and against one that answered nothing when the install was looked up, since neither will run the command and reporting that as a fault of what was probed fails a host whose agent and boundaries are fine. The refusal and the outage are reported by the secrets and socket checks instead.

They differ on a broker that was never asked. The brokered command check needs root, so an unestablished value set there is `--check` not having reported, which is what it says. The SSH agent probe runs as the caller's own account, so it is answered without sudo, and a refusal that comes back is recognised as one. A refusal from a broker whose `--check` read every managed file fails rather than skips: `--check` reads those files itself, so a daemon refusing what they cover is one that came up before they were written.

**Finding the install.** `doctor`, `init-project`, `uninstall`, `edit`, `rekey` and `logs` all act on an install they did not perform, and each finds it the same way: `--config-dir` (or `--config`) if you name one, then the running broker's own answer, then the `FARAMIR_CONFIG=` its unit names, then the compiled-in default. The unit is what covers a host whose config moved and whose broker is down. `init` finds it the same way and prints what it settled on before it writes anything: a re-run is normally re-provisioning the install that is already there, and falling to the compiled-in default instead would mint a second set of keys beside the first and point the units at them. Naming `--config-dir` is still what puts an install somewhere new.
...
`sudo faramir uninstall` | Removes the broker from the install it [finds the usual way](#checking-an-install). Leaves the accounts, the config, the secrets, the key and the audit log, and says so: deleting the age key would make every managed sops file unreadable, retroactively.
...
- **A brokered `ssh` verifies a host against `/etc/ssh/ssh_known_hosts`, or against the executor's own.** That file starts empty and stays empty, no prompt reaching you through a child whose stdin is `/dev/null`, so a host trusted only in your `~/.ssh/known_hosts` is refused before the broker's key is ever offered. `init --known-hosts ~/.ssh/known_hosts` pins yours for the executor, and `faramir doctor` reports how many host keys it can verify against and whether the executor can read them. The system-wide file is the other way, covering the executor, the operator and root at once and outliving a service account being recreated. Either way an entry is filed under the name ssh dials, bracketed with the port where that is not 22 (`[host.example.com]:2222`); under any other name it is never consulted. Seed the system-wide file over a connection you have already verified, reading the host's own public host keys rather than trusting what answers a bare `ssh-keyscan`:
  ...
  The removal first is what makes it re-runnable: appending alone leaves a rotated host's old key in the file as a second line ssh still accepts.

  Every type the host offers, not `ed25519` alone: the algorithm is negotiated per connection, so an entry for a type ssh does not pick verifies nothing. [ansible-ctrl's faramir role](https://github.com/andornaut/ansible-ctrl/blob/main/roles/faramir/tasks/ssh.yml) does this across a fleet, keeping entries a run did not reach and refusing a pinned name whose key has changed.
