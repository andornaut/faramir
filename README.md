...
**Credentials faramir does not manage.** An SSH private key, a `.pem`, a `.env`, an `~/.aws/credentials` | The deny rules cover this install's own paths and the age identities. Anything faramir does not write is yours to declare with [`faramir refuse`](docs/configuration.md#refused-paths), by path or by name, and an install that declares none refuses none
