...
Enrol with `faramir init-project --agent claude --agent gemini`, repeatable, defaulting to `claude`. The names are `claude`, `gemini`, `opencode` and `kilocode`. [Antigravity](https://antigravity.google/) is not supported: its hooks decide and cannot rewrite.
