...
`--ssh-key PATH` | the install's, then `<config-dir>/id_ed25519` | Where the keypair the broker lends lives. One is minted either way, so this relocates rather than enables. An existing key is adopted, and must be `faramir-broker`-owned `0600` with its `.pub` beside it at `0644`. Wherever it lands it is refused to the agent's tools and to a brokered command alike, the rule being rendered from the configured path rather than from the config directory that usually holds it
...
> [!IMPORTANT]
> **A credential faramir neither writes nor reads is yours to declare.** An SSH private key, a `.pem`, a `.env`, an `~/.aws/credentials`: none is refused by an install that declares nothing, so an agent's file tools can open them. `faramir block add --path` names one, and a directory covers everything under it ([blocked paths](configuration.md#blocked-paths)); `--strict` says a brokered command may not name one either, and `faramir block ls` shows both halves. A fleet declares them once in whatever converges its hosts.
