...
> [!IMPORTANT]
> **A credential faramir neither writes nor reads is yours to declare.** An SSH private key, a `.pem`, a `.env`, an `~/.aws/credentials`: none is refused by an install that declares nothing, so an agent's file tools can open them. `faramir block add` names one, `--name` names a class of them ([blocked paths](configuration.md#blocked-paths)), and `faramir block ls` shows both halves. A fleet declares them once in whatever converges its hosts.
