...
`blocked` | The command would read, copy or move something this host declares under `[[secret.block]]` or `[[secret.link]]`, or run a declared command. **Terminal, not retryable**: nothing about the host will change to make the same command allowed. Names the entry that matched. A command outside the reader and mover vocabulary is not refused, whatever it does to the file: see [the brokered route](configuration.md#the-brokered-route). `run` alone
