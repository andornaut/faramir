...
`blocked` | The command would print something this host declares under `[[secret.block]]` or `[[secret.link]]`, run a declared command, or name one of faramir's own directories, which no brokered command may do for any reason. **Terminal, not retryable**: nothing about the host will change to make the same command allowed. Names the entry that matched. A brokered command outside the reader vocabulary is not refused, whatever it does to the file, which is the broker's reading and not the guard's: see [the brokered route](configuration.md#the-brokered-route). `run` alone
