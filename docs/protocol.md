...
`blocked` | The command would read, copy or move something this host declares under `[[secret.block]]` or `[[secret.link]]`, or run a declared command. **Terminal, not retryable**: nothing about the host will change to make the same command allowed. Names the entry that matched. A command that only changes a declared file's mode or owner, or removes it, is not refused: see [the brokered route](configuration.md#the-brokered-route). `run` alone
