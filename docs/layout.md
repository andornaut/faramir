...
Pi gets no rule file, having nowhere to put account-wide rules: the same paths are compiled into the extension `init-project` installs. It gets the section like the rest. Antigravity gets no rule file either, its permission lists being the IDE's own state. Kilo Code has no single home instructions file, so its section is a file of faramir's own in the global rules directory, every `.md` in which is loaded for every project.
