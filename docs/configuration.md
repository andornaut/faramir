...
**A quoted heredoc body is data, not commands.** `<<'EOF'` and `<<"EOF"` make every line up to the terminator literal, so a script or a document written that way is not refused for the commands it quotes. Only the quoted spelling: a bare `<<EOF` expands `$(...)` and backticks in its body, which is a command, so those bodies are read as commands. A heredoc whose terminator never arrives leaves the line unreadable, and an unreadable line is matched whole, which refuses more rather than less.
...
**What is refused is a vocabulary, not a direction: the commands that read a file and the ones that move or re-encode it, wherever the declared path sits in the line. Everything else is left alone, writing over the file included.**
