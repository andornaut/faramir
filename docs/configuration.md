...
0. Resolves the path. A symlink becomes its target, because that is the file the grant and the group are applied to and the file whose mode every check below reads. The spelling you typed is written as a `[[secret.block]]` entry carrying `derived_from`, so the rules cover both names: a rule matches the path a command names, and the name your agent has is the one you typed. `link rm` takes that entry with the link, unless another link or a `[[secret.block]]` entry still names the target, in which case the entry stays. A typed spelling that is, or holds, an enrolled tree is not blocked, a rule there refusing the agent the directory it works in, and the report says so. Nothing is derived where the path named the file directly.
...
A `[[secret.block]]` entry (a block) keeps one thing away from the agent. Use it for a credential faramir never needs the value of: a LUKS keyfile, an SSH identity. See [when to use one](integrations.md#where-the-value-lives).
...
- **A symlink is blocked at both names.** A rule matches the path a command names, and a link and its target are two names for one file, so an entry for the link alone leaves the file readable under the target's name. `block add` resolves the path it is given and records the target as a second entry, carrying `derived_from` and the strictness of the entry it came from. Both are written into `config.toml`, so the rules are a function of the config alone and `block ls` shows what is in force. `block rm` on the declared path takes the derived entry with it, unless another declared symlink still resolves to the target, in which case the entry stays and `derived_from` names that one; an entry that cannot be read is taken to resolve to it, a rule kept for nothing costing a row where a rule dropped costs the file. The resolution happens where the entry is declared, not where a command is matched: a symlink created *after* the entry was written is not covered until the next `block add`, and a target that is, or holds, an enrolled tree is left underived and reported, a rule there refusing the agent the directory it works in. A symlink repointed since it was declared keeps its entry for the old target until the next `block add` naming the link, which replaces it, and `faramir doctor` reports the drift as `derived paths` until then. An operator who declares the target on its own account takes that entry over, and it then survives a removal of the link. A `[[secret.link]]` entry derives one too, in the other direction: `link add` resolves a symlink to the file that holds the value and blocks the spelling that was typed, `derived_from` naming the link's own path, and `link rm` takes it under the same rule. An entry that goes is a source taken away in turn, so what was derived from it goes or stays the same way. A derived entry is removed only by the entry that wrote it, so a `block rm` naming a path this config does not block removes nothing and leaves the derivation standing.
