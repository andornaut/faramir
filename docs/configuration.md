...
- **Every run is confined to a cgroup and reaped there, when one is available.** Not a setting: `init` renders `Delegate=` on the executor unit so a real host confines. On an *elevating* host it is mandatory — with no delegated cgroup (needs cgroup v2 and `cgroup.kill`, kernel ≥ 5.14) the executor refuses to run rather than reap by process group, which a `setsid` child escapes; elsewhere the process-group kill covers a run that cannot be confined. `faramir doctor` fails the first case and warns the second.
