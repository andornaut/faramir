...
There is one config file, `<config-dir>/config.toml`, and faramir owns it. `faramir init` renders it from [etc/config.toml.tmpl](../etc/config.toml.tmpl) on every run. There are no drop-in files: every value is set by a flag, written by `faramir link` or `faramir block`, or derived from the install.
...
It is not for `--command`, which is already matched wherever a command starts. `block add --strict` applies to the `--path` entries it names and not to its `--command` entries, and is refused when it names no `--path`. A `[[secret.block]]` entry carrying both `command` and `strict` is refused at load.
...
A managed file that was found and did not load | Its values are in no redactor, so a command that prints one prints it in the clear
A ref two managed files define with different values | One value wins. The other is on disk and in no redactor
