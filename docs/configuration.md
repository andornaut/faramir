...
- **Every run is confined to a cgroup and reaped there.** Not a setting: `init` renders `Delegate=` on the executor unit so a real host confines. The cgroup is the one reaper, with no process-group fallback — with no delegated cgroup (needs cgroup v2 and `cgroup.kill`, kernel ≥ 5.14) the executor refuses to run rather than reap by process group, which a `setsid` child escapes. This holds on every host, a sudo grant or not, and `faramir doctor` fails a host whose executor unit is not delegated one.
