...
**The whole of `[sudo]` but `timeout_sec` is `init`'s, because it is the approval boundary itself, decided per host at install time.** Whether the section is rendered at all is the `--allow-sudo` flag ([operating.md](operating.md#the-decision-is-made-at-init-per-host) has the decision): unset, there is no `[sudo]` section, no sudoers entry and no PAM service, and a brokered command's `sudo` fails as on any host that granted nothing. Set, `exec_user` is the account the grant is written for, `pam_service` names the file `sudo` authenticates it through (so a drop-in pointing it at a service the operator wrote would choose what decides every approval), and `helper` is the program PAM execs, as root, to make that decision. A drop-in setting any of them is refused; re-run `init --allow-sudo` to change the arrangement, or re-run without it to take the grant away. `timeout_sec` is a plain default and yours: how long a question waits for a human before it is refused, 120s by default and **bounded between 1 and 600**. The ceiling is not a taste. The PAM helper has to outlast any question the broker will hold, or it gives up on one still open and the operator's yes lands on a `sudo` that has already gone; it cannot read this config (PAM gives it no environment and its argv is fixed at install time), so it derives its own deadline from the same ceiling and the two cannot drift. While a question is open, `sudo` blocks and every other brokered command on the host is refused, so past ten minutes a refusal and a second run is the better answer. `notify_command` is unset by default and is `init`'s only because the broker execs it; it announces a pending question and carries no answer. Its `{prompt}` is the rendered question, safe to put on a terminal; its `{id}` is what an operator types to answer, so keep it off anything that broadcasts: `wall` reaches every terminal on the host, and the coding agent has one.
