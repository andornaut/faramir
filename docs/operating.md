...
`faramir block ls` | Everything blocked here and where each rule is enforced. A table of the entries this install declares and the directories the install itself occupies, each covering the agent's file tools and the command guard alike, and under it the command rules faramir carries itself, which no entry changes. The directories are the part nothing else can be asked about: they are derived from the layout rather than written anywhere an operator would read. Both are listed because nothing else can be asked: a refusal names the rule that matched rather than the set, which is how a rule that covers something gets reported as a gap. `--declared` narrows it to the entries the config carries, which is the list a configuration manager converges and the one that prints `[]` when there are none. `--json`
