...
Agents | `agent rules`, `agent rule drift`, `install rules`, `tree config`, `agent file ownership` | Each agent's deny rules present, absent, or carried in an extension; rules an earlier version wrote that this one does not; the account-wide rules still carrying the paths this install writes, restored by re-running `init`; enrolled trees whose agent files no longer carry what the enrolment wrote; and files an install would now refuse to write
