...
Refused paths | `refused paths`, `unrefused credentials` | Every `[[secret.refuse]]` entry carried by the agent's deny rules, which is the whole of what one of those entries does. Whether the path is there is not asked: an entry for a key on an unmounted volume is doing its job. Beside it, what **is** there and refused by nothing: faramir refuses what it installs and leaves the rest to the operator, so this names the well-known credential files it finds in the agent's home with no rule over them, and the command that would name them. A warning, never a failure, and nothing is opened
