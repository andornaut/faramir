...
sops | `sops config`, `rule coverage` | `.sops.yaml` lists the keeper's own recipient rather than one it used to have, and lists nothing sops would refuse; and its creation rule reaches every file `[secrets] patterns` names
...
Nothing checks what you type into that file as you type it, so run `sudo faramir doctor` after: `sops config` puts every entry through the same validation `--age-recipient` gets at install, which is what catches an identity pasted where a recipient belongs. The file is `0644`, so a private half written there is the key that opens the secrets directory, readable by every account on the host; if one lands there, treat it as disclosed and rotate.
...
- **A `.sops.yaml` with more than one creation rule is refused**, the recipients then depending on which `path_regex` a file matches. The rules are parsed rather than pattern-matched, so the count holds however they are written: keys in any order, flow style, `age:` as a string or a list. Use `sops updatekeys` per file, which is the only thing that can answer which rule governs which.
- **The rule is `<config-dir>/.sops.yaml` and there is no flag naming another.** Both commands hand sops that file and match it against the managed file's real path rather than the tmpfs copy the plaintext passes through. Left to search, sops walks up from the directory you were standing in, which may be a working tree the coding agent writes: a `.sops.yaml` there would decide how the managed file is written, and `unencrypted_regex` in one puts the values it names into that file in cleartext. Sealing to some other file's recipients is the state `rekey` exists to remove, `doctor` and every file sops creates afterwards still reading the install's rule; `--config` moves the whole install, which is how to act on another one. A host whose `.sops.yaml` has been removed can still be edited, `edit` falling back to sops' defaults; `rekey` stops before it opens anything, that file being where it reads the recipients to re-encrypt to.
- **A file no creation rule covers cannot be written back**, sops refusing one it has no rule for. `edit` asks before it opens the editor rather than after, so a store the rule does not reach costs a refusal rather than everything you typed; `doctor` reports the same question across every managed file under `rule coverage`. The shipped rule matches any `*.sops.yml` or `*.sops.yaml` wherever it sits, so this arrives only where the rule was narrowed or `[secrets] patterns` names something else.
