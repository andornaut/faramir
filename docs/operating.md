...
Command | Does
--- | ---
`sudo faramir init-project [DIR]` | Enrols one working tree, `DIR` defaulting to the current working directory: [shares the tree](layout.md), writes the credentials section into the tree's agent instructions file and into the file of its own that Claude Code and Antigravity read ([which name](layout.md)), and for Claude Code registers the routing hook in `.claude/settings.local.json`. The deny rules are `init`'s and hold account-wide. The installed `config.toml` has to be readable, the linked and blocked paths among those rules being only there. A home directory, `/`, anything above a home, the system directories (`/etc`, `/usr`, `/var` and their kind) and faramir's own directories are refused, symlinks resolved first
