...
- **The credentials section is written once and never edited.** `init-project` adds it to the tree's agent instructions file when that file shows no sign of faramir, and leaves it alone otherwise, naming anything else in a warning. Delete the section and re-run to get the current one.
