...
- **The credentials section is delimited, and everything between the markers is faramir's.** `init-project` writes it into the tree's agent instructions file, and `init` writes a shorter one into each agent's home instructions file ([which file, per agent](layout.md)), both between `<!-- BEGIN faramir: credentials -->` and `<!-- END faramir: credentials -->`. A later run replaces what is between them whatever it now says, and text outside them is untouched. Three kinds of file are left exactly as they are and named in a warning: one carrying a single marker, where the block stops not being readable off it; one already carrying a credentials section that is not between markers and is not what is written now, which would otherwise end up with two sets of instructions contradicting each other, and which is fixed by deleting that section once; and a symlink, which writing would replace with a regular file. A section whose markers were stripped but whose text is untouched is wrapped again where it stands.
...
- **A brokered command cannot delete or rename an operator-owned file at the top level of an enrolled tree.** The tree root and each agent's own directory carry the sticky bit, so unlink and rename there belong to the file's owner. It is what stops a brokered command replacing the file that names the hook, unlink being a permission on the directory and not on the file. The cost lands on tools that rewrite a root-level file by rename, a lock file or `go.mod` among them: they get `EPERM`. Writing through an existing file still works, as does anything in a subdirectory that is not an agent's.
