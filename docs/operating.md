...
`sudo faramir block add --strict` | Rides on every `--path` and `--name` the same command names, and refuses every command *naming* them rather than the ones that would read them. Refused on `--command`. `faramir link add` takes the same flag. [Detail](configuration.md#refusing-every-mention-of-an-entry)
