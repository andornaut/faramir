...
`sudo faramir block add --strict` | Rides on every `--path` the same command names, and refuses every command *naming* them rather than the ones that would read them. Not for `--command`, which already matches wherever a command starts; as a key on a hand-written command entry it is refused. `faramir link add` takes the same flag. [Detail](configuration.md#refusing-every-mention-of-an-entry)
