...
`sudo faramir refuse rm PATH` | Drops the entry, so `init` stops rendering the rule. It does not take the rule out of an agent's file, a merged rule file only being addable to, and says so. A path this install does not refuse is not an error. `--name` removes a name entry; the form is part of what identifies one, so a name is not removed by giving the same string as a path. `--json`
