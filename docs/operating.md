...
Files | `config ownership`, `config reach`, `installed files`, `deny patterns` | The config, `.sops.yaml`, the binary, `wrap.sh` and the PAM helper are not writable by the operator; the broker's account can read the config (a reload needs this); the command guard's rule file matches what this install renders. `deny patterns` re-renders the file and compares: a rule missing from the host refuses less than the config asks and fails; a spare rule warns. Every rule is compiled first, because the hook skips a rule it cannot compile and a re-render would not notice. Comments are not compared
...
Blocked paths | `blocked paths` | Every `[[secret.block]]` path is refused by the agent's deny rules. The broker's own refusal of such a path is daemon behaviour and is not checked here. Whether the path exists is not asked: an entry for a key on an unmounted volume is still correct
...
`sudo faramir link rm REF` | Removes the entry, so the value leaves the redactor. Undoes neither the grant nor the deny rule, and prints both. `--json`. [Detail](configuration.md#linked-secrets)
...
`sudo faramir block add --command COMMAND` | Blocks a command, written as it would be typed (`op read`). Reaches the command guard and the broker; a file tool cannot name a command. [Detail](configuration.md#what-each-form-accepts)
...
- **Run `init` before enrolling a project with opencode, Kilo Code or pi.** Their plugins fail closed, so a binary too old to know the agent refuses every command in that project instead of running it unredacted. [What those plugins ask the guard](coding-agents.md#opencode-kilo-code-and-pi).
...
`init` seals the store to one key: the keeper's own, minted on the host. It writes `.sops.yaml` once and keeps it on every later run, reading it back and reporting its recipients as `age_recipients`. Re-running the installer never changes a recipient. `faramir vault edit` does not apply a changed rule either: it re-encrypts to the recipients the file already carries, so an edit cannot drop a reader.
...
A second recipient and a backup cover different losses. Another reader keeps the *values* readable if this host's key is gone; the [archive below](#backing-up-and-restoring) rebuilds *this host*. Neither replaces the other.
...
- **Dropping a recipient does not reach a copy already held elsewhere.** Treat what that key could read as read.
