...
Daemons | `sockets`, `unit drop-ins`, `broker`, `version`, `protectproc`, `secrets store` | Are the socket units listening and enabled, does a drop-in override a unit, does `--check` pass, do the CLI and the running broker report the same build, do the units keep `ProtectProc=invisible`, and what the store the broker read holds. Keeping no managed file is a warning rather than a failure: a `[[secret.link]]` entry fills the value set on its own, and a host that has not written its first secret is every install on its first day. A file that is there and did not load stays a failure
Store health | `refused refs`, `shadowed refs`, `linked refs` | Refs the store read and the redactor refused, so they are never injected and never redacted; a ref two managed files define differently, which leaves one value in no redactor; and linked files that did not load, whose refs answer nothing while every other ref is served
Memory | `memory bounds`, `broker memory` | The executor's per-process bound reachable within its cgroup total, and what the broker holds against what it may hold, both read from systemd because the limits resolve against the cgroup's own numbers
...
Blocked paths | `blocked paths` | Every `[[secret.block]]` path refused by the agent's deny rules. The broker holds the same entries and refuses a brokered command that would read one, which is not a check: it is the daemon's own behaviour and needs nothing asserted about the agent's files. Whether the path is there is not asked: an entry for a key on an unmounted volume is doing its job
...
Agents | `agent rules`, `hook reach`, `agent code`, `agent rule drift`, `install rules`, `tree config`, `tree modes`, `agent file ownership`, `codex hook trust` | Each agent's deny rules present, absent, or carried in an extension; which tools each registration of the guard answers for, one matching less than every tool leaving that agent's file tools reaching the guard through nothing; the plugin and hook files still carrying what `init` writes, each being the whole of what routes or refuses for its agent; rules an earlier version wrote that this one does not; the account-wide rules still carrying the paths this install writes, restored by re-running `init`; enrolled trees whose agent files, credentials sections and rules frontmatter still carry what the enrolment wrote, at their own modes in sticky directories; files an install would now refuse to write; and whether Codex has been told to trust the hooks written for it
...
Four statuses: `ok`, `warn`, `failed`, and `n/a` for a check whose subject this install does not have, a separate total because a pass would claim a stack that is not there. A run that has to stop early, on a missing config or an account that does not resolve, accounts the rest under `examination` as unasked, so one failure never reads as a host where everything else passed. Every probe runs on a deadline, a hung `systemctl` or broker reported as unasked rather than hanging the examination.
...
- **`codex hook trust` fails on a hook Codex will not run**, which is the one thing an enrolment installs that nothing else reports. Codex skips a hook it has not been told to trust and says nothing when it does, so an unguarded Codex is indistinguishable from a guarded one until this is asked. It reads `~/.codex/config.toml` and compares what Codex recorded against the identity Codex computes for each hook on disk, so a hook trusted before a release rewrote it reads differently from one never trusted. Granting the trust is an answer to Codex's own prompt: no flag or config key does it, so the remedy is always to start Codex once where the hook is.
...
- **Three checks run a brokered command** rather than reading a mode: `ssh agent`, `brokered command` and `redaction`, the last sealing a synthetic probe value into the store, expecting exactly its token back, and removing it. The first two skip against a broker known to hold no values. `brokered command` and `redaction` need root; the `ssh agent` probe runs as the caller. A refusal from a broker whose `--check` read every managed file fails rather than skips: a daemon refusing what those files cover came up before they were written.
...
**Without the agent's account**, most boundary checks cannot be put at all: `access(2)` answers "no" for an account that cannot be named, which is the same answer a boundary that holds gives. Run from a root shell or cron, `doctor` takes that account from `$FARAMIR_OPERATOR`, then `SUDO_USER`, then the calling account, then `[server] agent_user`, refusing root and faramir's own accounts at every step. It reports those checks as unasked only where none of the four names one, which is a host `init` has not finished: `faramir init --agent-user` is what records it, `doctor` taking no such flag.

**Finding the install.** `doctor`, `enrol`, `uninstall`, `link`, `block`, `vault add`, `vault edit`, `vault ls`, `vault rm`, `reader` and `logs` all act on an install they did not perform. None of them takes the path: a caller cannot be expected to know where the config lives, and every one of them can ask. One ladder, whether the command wants the directory or the file.
...
`enrol --dry-run` is the other exception, and carries on without an install: it writes nothing, so it has no wrong install to act on, and reporting on a tree from a host not provisioned yet is what it is for.
...
- **One file twice.** Two paths in the same run that a symlink makes one file, such as `~/.gemini/GEMINI.md` pointing at `~/.claude/CLAUDE.md`. Each is written for the agent that reads it, so one file standing in for two would keep only the last write and report success. Point one at a file of its own. Two cases are not this: two agents that read the same file *by name* get one file written once, its section claiming only what holds for both; and a tree's instructions files all carry the same section, so a `CLAUDE.md` linked at the tree's `AGENTS.md` is written once rather than refused.

Each is asked before anything is written, so a refusal costs nothing: `init` stops before it has handed a file to any account, `enrol` before it has shared the tree. `init` names every file it refused rather than the first. `doctor` asks the same questions under `agent file ownership`.

The section tells an agent to wait for an escalation only where one can be raised, `enrol` reading `[sudo] exec_user` from the config.
...
Command | Does
--- | ---
`sudo faramir enrol [DIR]` | Enrols one working tree, `DIR` defaulting to the current working directory: [shares the tree](layout.md), writes the credentials section into the tree's agent instructions file and into the file of its own that Claude Code, Codex and Antigravity read ([which name](layout.md)), and registers the routing hook for the two agents whose hook approves as well as rewrites: Claude Code's in `.claude/settings.local.json` and Codex's in `.codex/hooks.json`. The deny rules are `init`'s and hold account-wide. The installed `config.toml` has to be readable, the linked and blocked paths among those rules being only there. A home directory, `/`, anything above a home, the system directories (`/etc`, `/usr`, `/var` and their kind) and faramir's own directories are refused, symlinks resolved first
...
`sudo faramir vault rm NAME` | Takes a file out of the store, naming the refs it will destroy and asking `[y/n]`; `--force` answers for a script. The audit record keeps the refs it held
...
`sudo faramir link add REF FILE` | Reads a secret out of a file another tool maintains, instead of copying it in; `--type` and `--key` say how. `REF` is the name a caller asks by, with or without the `faramir://` that `faramir refs` prints it with. Grants the broker read, refuses the file to the agent's file tools, writes the entry and reloads. `--json`. [Detail](configuration.md#linked-secrets)
`sudo faramir link rm REF` | Drops the entry, so the value leaves the redactor. Undoes neither the grant nor the deny rule, and prints both. `--json`. [Detail](configuration.md#linked-secrets)
`faramir link ls` | The linked secrets this install declares, and whether each file is there. `--json` prints the entries as the config carries them, without the state column. [Detail](configuration.md#linked-secrets)
`sudo faramir block add --path PATH` | Refuses one path to the agent's file tools, to its shell, and to a brokered command that would print it, without opening it. Writes the entry, re-renders the rules and reloads, and nothing else: no grant, no mode change, no value in the redactor. `--json`. [Detail](configuration.md#blocked-paths)
`sudo faramir block add --command COMMAND` | Blocks a command, written as it would be typed (`op read`). Reaches the command guard and the broker, a command being nothing a file tool can name. [Detail](configuration.md#what-each-form-accepts)
`sudo faramir block add --strict` | Rides on every `--path` the same command names, and narrows the **brokered** route to refusing every command *naming* them rather than the ones that would print them. It changes nothing for the agent's own shell, which refuses a declared path named at all either way. Not for `--command`, which already matches wherever a command starts; as a key on a hand-written command entry it is refused. `faramir link add` takes the same flag. [Detail](configuration.md#refusing-every-mention-of-an-entry)
`sudo faramir block rm --path PATH` | Drops the entry, so `init` stops rendering the rule. `--command` removes a command entry, the form being part of what identifies one. Repeatable, as `add` is. `--json`. [Detail](configuration.md#how-the-entries-behave)
`faramir block ls` | Everything blocked here: the declared entries in a table of kind and entry, and under it the rules faramir carries itself. `--declared` and `--built-in` narrow it to one half or the other. `--json`. [Detail](configuration.md#blocked-paths)
...
At the broker these four commands are two ops, `ls` and `watch` both asking `escalations`, and `approve` and `reject` both being `answer` with a different verdict: `escalations`, `answer` and `escalate` are root-only there too, checked with `SO_PEERCRED`, so the account the coding agent runs as cannot answer what the agent asked for. `escalate` is the one sudo's PAM helper asks, and so the one that decides whether a brokered command becomes root.
...
- **Interactive prompts fail rather than hang.** Stdin is `/dev/null` unless the caller piped something in with `faramir run -i`, and the child gets no controlling terminal either way, so a prompt falls back to stderr, which is redacted and recorded; one written only to `/dev/tty` is lost ([why](redaction.md#why-a-pty-and-not-a-pipe)). A prompt that reads piped input gets the caller's first line rather than a passphrase anybody typed. Pass non-interactive flags.
