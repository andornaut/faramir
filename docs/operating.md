...
`sudo faramir block rm PATH` | Drops the entry, so `init` stops rendering the rule. A rule compiled into faramir is refused rather than removed, whether it is named as a pattern or as a file it covers: it is not an entry, and the host goes on refusing it. It does not take the rule out of an agent's file, a merged rule file only being addable to, and says so. A path this install does not refuse is not an error. `--name` removes a name entry; the form is part of what identifies one, so a name is not removed by giving the same string as a path. Repeatable, as `add` is. `--json`
`faramir block ls` | Everything blocked here and where each rule is enforced: four columns of source, kind, entry and which entry points carry it. The entries this install declares and the directories the install itself occupies, and under them the command rules faramir carries, which no entry changes. The directories are the part nothing else can be asked about, being derived from the layout rather than written anywhere an operator would read. Whether a declared path is there today is `--json`'s `state`, not a column. Both are listed because nothing else can be asked: a refusal names the rule that matched rather than the set, which is how a rule that covers something gets reported as a gap. `--declared` narrows it to the entries the config carries, which is the list a configuration manager converges and the one that prints `[]` when there are none. `--json`
