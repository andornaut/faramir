...
`sudo faramir refuse rm PATH` | Drops the entry, so `init` stops rendering the rule. It does not take the rule out of an agent's file, a merged rule file only being addable to, and says so. A path this install does not refuse is not an error. `--json`
`faramir refuse ls` | The paths this install refuses, and whether each is there. `--json` as above; an install declaring none prints `[]`
