...
`sudo faramir block add --strict` | Rides on every `--path` and `--name` the same command names, and refuses every command *naming* them rather than the ones that would read them: `ls`, `stat` and `chmod` included, in the agent's shell and through the broker alike. For a directory the agent has no business in at all. Nothing converges a path declared this way, so it is not for a key something has to rotate. Refused on `--command`, which is already matched wherever a command starts. `faramir link add` takes the same flag. [Detail](configuration.md#refusing-every-mention-of-an-entry)
