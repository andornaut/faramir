...
Agents | `agent rules`, `agent code`, `agent rule drift`, `install rules`, `tree config`, `tree modes`, `agent file ownership`, `codex hook trust` | Each agent's deny rules present, absent, or carried in an extension; the plugin and hook files still carrying what `init` writes, each being the whole of what routes or refuses for its agent; rules an earlier version wrote that this one does not; the account-wide rules still carrying the paths this install writes, restored by re-running `init`; enrolled trees whose agent files, credentials sections and rules frontmatter still carry what the enrolment wrote, at their own modes in sticky directories; files an install would now refuse to write; and whether Codex has been told to trust the hooks written for it
...
`sudo faramir block add --path PATH` | Refuses one path to the agent's file tools, to its shell, and to a brokered command that would read, copy or move it, without opening it. Writes the entry, re-renders the rules and reloads, and nothing else: no grant, no mode change, no value in the redactor. `--json`. [Detail](configuration.md#blocked-paths)
...
`sudo faramir block add --name PATTERN` | The same, against a name rather than a path: matched against what the agent names rather than against this host, which is what reaches a path the host does not have. Each flag given is one entry, and the three forms mix in one command. [Detail](configuration.md#a-path-and-a-name-are-different-rules)
`sudo faramir block add --strict` | Rides on every `--path` and `--name` the same command names, and refuses every command *naming* them rather than the ones that would read them. Not for `--command`, which already matches wherever a command starts; as a key on a hand-written command entry it is refused. `faramir link add` takes the same flag. [Detail](configuration.md#refusing-every-mention-of-an-entry)
`sudo faramir block rm --path PATH` | Drops the entry, so `init` stops rendering the rule. `--name` removes a name entry, the form being part of what identifies one. Repeatable, as `add` is. `--json`. [Detail](configuration.md#how-the-entries-behave)
