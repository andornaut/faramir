...
2. **Commands naming a protected path are refused.** [deny-patterns.txt](../agent/hooks/deny-patterns.txt) names the paths the install protects, and a command naming one is refused whatever it would do with it. There is no verb list to be short of: every subject is an absolute path, and a path names itself. One rule per kind of entry, each naming its kind, so the refusal can name the command that takes that entry back. Faramir's own binary, units and plugin files keep verb rules, because an agent reads and runs those and only replacing them is refused.
