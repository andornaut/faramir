...
2. **Leak-prone commands are refused.** [deny-patterns.txt](../agent/hooks/deny-patterns.txt) names readers, writers and redirects pointed at anything the install protects, plus faramir's own binary, units and plugin files.
   ...
   What an operator declares, under `[[secret.block]]` or `[[secret.link]]`, is enforced twice, because the deny rules reach the agent's own tools alone: the broker holds the same entries and refuses a brokered command that would read, copy or move one, `faramir_run` being the route no hook over shell tools can see. Changing a declared file where it stands is left alone, see [the brokered route](configuration.md#the-brokered-route).
