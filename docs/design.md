...
A home is not mounted until its owner logs in, so the secrets are absent at boot and to cron. Absent is therefore never quietly tolerated, and one rule covers every way of being short of them: **the broker serves `exec` and `redact` only while it holds every value the config asked for.** Nothing configured, an entry that named no file, a file that is there and did not load, all refuse. The daemon comes up regardless and says which it is, because `faramir status` and `faramir doctor` are what explain a missing store and both need the process it is asking. `--check` and `doctor` fail on all of them; the daemon refusing per request is what makes coming up safe.
