...
2. **Leak-prone commands are refused.** [deny-patterns.txt](../agent/hooks/deny-patterns.txt) names readers, writers and redirects pointed at anything the install protects, plus faramir's own binary, units and plugin files. The first half is generated from the same set the agents' deny rules are: one list, two entry points, so a declared path is refused to a file tool and to `cat` together rather than to whichever was thought of. A path under a home is named in the spellings a shell expands to it, `~/` and `$HOME/` among them, that being how the file is usually written rather than a way around the rule. The refusal states the alternative, because a denial the agent cannot act on gets worked around. Nothing is refused for being a decryption or an environment dump: what a command *does* is the operator's to declare, `block add --command`, and in an enrolled tree the command is rewritten before it runs, so `printenv` comes back with every managed value replaced by its token.
...
`faramir doctor` fails on either. Everything else fails closed by construction: an unreachable broker, a request no run owns, a refusal and a timeout all exit non-zero, and nothing authenticates except by reaching `pam_permit` past a `requisite` that succeeded. The service is faramir's own, so a mistake in it reaches the executor and no other account: everyone else's `sudo` authenticates the way it did before the install. What sends the executor there is `pam_service=` in the sudoers entry on a host running the original sudo, which touches nothing shared, and a delimited block in `/etc/pam.d/sudo` on one running sudo-rs, which has no such setting and so no service file at all ([the two sudos](escalation.md#the-two-sudos)). The branch tests the account and falls through for everybody else, so the reach is the same and the exposure is not: it is a file the distribution owns, and `doctor` re-checks that it still says what it should. Removing the file falls back to `/etc/pam.d/other`; `doctor` fails if that fallback is a free pass.
