...
`faramir doctor` fails on either. Everything else fails closed by construction: an unreachable broker, a request no run owns, a refusal and a timeout all exit non-zero, and nothing authenticates except by reaching `pam_permit` past a `requisite` that succeeded. The service is faramir's own, named by `pam_service=` in the sudoers entry, so everyone else's `sudo` reads the stock `/etc/pam.d/sudo` and a mistake here can neither lock this host's operators out nor hand another account a free root. Removing the file falls back to `/etc/pam.d/other`; `doctor` fails if that fallback is a free pass.
