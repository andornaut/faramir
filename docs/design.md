...
A home is not mounted until its owner logs in, so the secrets are absent at boot and to cron. Absent is therefore never quietly tolerated, and the two ops ask the store a different question about it.

**One question for both.** A brokered command's output is redacted against the same value set, so what makes `redact` unsafe makes `exec` unsafe: output holding a managed secret the redactor does not have. What matters is whether a managed file exists whose contents went unread, so at least one file matched and every matched file loaded. How many secrets came out of the files that were read does not: a file that is there and empty holds nothing to miss, and an install whose operator has not written a secret yet serves. A ref no file defines is answered by `unknown_secret`, which says the true thing where this gate would say a vague one. Nothing configured and an entry that named none are refused alike rather than told apart, since the file may be absent because it was never written or because the filesystem holding it is not mounted, and only the second is dangerous. That costs a fresh install every wrapped command until it has one file, which is loud and says why. The daemon comes up regardless and says which it is, because `faramir status` and `faramir doctor` are what explain a missing store and both need the process it is asking. `--check` and `doctor` fail on all of them; the daemon refusing per request is what makes coming up safe.

Checked per request rather than once at startup. A startup gate only ever judged the host as it was at boot: a reload that shrank the set afterwards left the broker serving a set it knew was short, with nothing to say so. The one exception is a keeper that could not be reached, where the set kept is the last one known to be true, so it is unconfirmed rather than short and a keeper hiccup does not become refused commands. A cold start needs no exception, holding nothing. That second half is what makes this placement survivable at all — `--check` runs from `init` and `doctor`, neither of which is around at boot, which is exactly when an unmounted home is absent. `init` also refuses to write into an unmounted encrypted home, which lands in the backing directory and is shadowed the moment the home mounts.
...
It is not rate-limited, and a limit that was there has been removed. Nothing short of removing the op closes the oracle, and a throttle slowed a guessing attack the same caller need never mount: `list_secrets` and `run` are ops on this socket behind the same `allowed_group` check, so every managed value can be had by naming it. Bounding the slower path bought nothing while the faster one is open by design, and cost a lock on the path the wrapper takes once per Bash command. Every call is still recorded, refused or not, so the visibility it was also credited with comes from the audit log rather than from the limit. A guess shorter than `[secrets] min_length` was never an oracle in the first place, every value below that length being refused at load and absent from the matcher.
