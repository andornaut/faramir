...
Filesystem isolation | None beyond file modes and `ProtectSystem=strict`, which `--allow-sudo` drops. | A home the executor may not read is one the mode already refuses; one it may read, the agent can read directly. Faramir's own directories are the exception, and are refused by rule rather than by mode: an approved escalation runs as root, where a mode refuses nothing.
...
**A link that is there and will not read is held to the same gate as a managed file that did not decrypt**, because it is the same state: a value on disk that the redactor does not have.
...
2. **Commands naming a protected path are refused.** [deny-patterns.txt](../agent/hooks/deny-patterns.txt) names the paths the install protects, and a command naming one is refused whatever it would do with it. There is no verb list to be short of: every subject is an absolute path, and a path names itself. One rule per kind of entry, and every rule names its kind, so the refusal can say which of them it is and name the command that takes that entry back. A `--strict` entry is a kind of its own, that being the one difference the two tiers have about an entry: its refusal names the ref rather than offering a brokered route the broker will not give it. Faramir's own binary, units and plugin files keep verb rules, because an agent reads and runs those and only replacing them is refused. The file is in kind order, which is the order the broker holds its own rules in: first match wins on both sides, so a command that is both an operator command and a named path gets the same answer either way.

   The first half is generated from the same set the agents' deny rules are: one catalogue, three entry points, so a declared path is refused to a file tool, to `cat` and to a brokered command together rather than to whichever was thought of. A rule in it carries what it is about, and each tier turns that into its own shape and its own sentence: the guard packs the subjects into one pattern per kind, a file of patterns having nowhere to keep a message, and the broker keeps a rule per entry so a refusal can name the entry and the command that takes it back. A path under a home is named in the spellings a shell expands to it, `~/` and `$HOME/` among them, that being how the file is usually written rather than a way around the rule, and by its tail alone where that tail is a path rather than a word, a rule having no working directory to follow a `cd` with.
   ...
   What an operator declares, under `[[secret.block]]` or `[[secret.link]]`, is enforced twice, because the deny rules reach the agent's own tools alone: the broker holds the same entries and refuses a brokered command that would print one, a brokered command being one that runs as another uid on the far side of the broker, where no hook over shell tools reaches. That vocabulary is the broker's own, and the only one left: a brokered command has to be able to use a credential file, so what it refuses is the commands that print its contents, and changing or moving the file is left alone. It holds this install's own directories as well, and the key the broker lends wherever `--ssh-key` put it, and those it refuses to any brokered command that names one, for any reason: that route runs as root wherever an escalation was approved, and root does not read a file mode. The commands that act on the install rather than through it go with them, `faramir vault edit` and `systemctl stop faramir-broker` among them: they are the operator's by either route, and the account on the far side of the broker is not the operator either. See [the brokered route](configuration.md#the-brokered-route).
...
**A `redact` op is an oracle.** A guessed value comes back confirmed or not. Acceptable only on weighting: an accident does not guess. It is not rate-limited, because a throttle bounds only a guessing attack the same caller need never mount: `refs` and `run` sit on the same socket behind the same `allowed_group` check, so every managed value can be had by naming it. Every call is recorded, and a guess shorter than `[secret] min_length` is not an oracle at all. Bring a limit back if `redact` ever becomes reachable by a caller that cannot reach `run`.

## Allowing sudo on the controller

How to install and run this is in [escalation.md](escalation.md); this is the reasoning.

**An escalation is the same kind of oracle as `redact`, weighted the other way.** A human can answer wrongly, so it survives that weighting by being deny-by-default: silence is a refusal, an unreachable answer channel is a refusal, and anything that is not `y` is a refusal.

An answer is worth something only if the question names the right command. The broker writes the question itself, from the argv of the command it started, and refuses any request it cannot match to a running command. The prompt string `sudo` hands the helper is ignored, because that string comes from the child, which is the side being asked about.
...
The service is faramir's own, so a mistake in it reaches the executor and no other account: everyone else's `sudo` authenticates the way it did before the install. What sends the executor there depends on the host ([the two sudos](escalation.md#the-two-sudos)). Under the original sudo it is `pam_service=` in the sudoers entry, which touches nothing shared. Under sudo-rs it is a delimited block in `/etc/pam.d/sudo`, which is a file the distribution owns: the branch tests the account and falls through for everybody else, so the reach is the same and the exposure is not, and `doctor` re-checks that the block still says what it should.

Removing the file falls back to `/etc/pam.d/other`, and `doctor` fails if that fallback is a free pass.

### What the escalation does not reach

Two ways it could go past the one command it named, with different answers.

**A second, unapproved command riding it: closed.** Every brokered command runs as `faramir-exec`, so whatever attributes a `sudo` to a run cannot be something the command holds: a value in an environment is readable within a uid, and one process of that uid hands it to another. Nothing is held. A run is named by the process the executor forked, and a `sudo` is attributed by its own ancestry, which the kernel supplies and no process chooses for itself.

What is still reachable is the approved run's own tree. A same-uid process that can `ptrace` into that tree is inside the run as far as an escalation is concerned, which is what `ptrace_scope` and `PR_SET_DUMPABLE=0` below are for.

**On top of that the broker serialises.** An escalation takes only when its run is the sole brokered command in flight, and while one is live every other brokered command is refused `escalation_in_progress`. That refusal is terminal rather than a `busy` to retry: a caller retrying against a live escalation would poll the exact interval the serialisation protects. Registering a run blocks a new escalation and a live escalation blocks a new registration, under one lock. A merely *pending* question holds a new command too, or a caller free to keep starting commands decides whether the host is ever quiet enough for a yes to take. The cost is that one unanswered question stalls unrelated brokered work for up to `[sudo] timeout_sec`.

Four things follow:

- **A pid names a run only while its holder is alive.** Attribution is by ancestry, so the executor has to match a pid the helper walked against what it forked, and a pid is the kernel's to hand on once the process wearing it is reaped. What settles it is a pidfd taken before the exec, which names the process rather than the number ([owner](protocol.md#the-executor-socket)). A number that went to something else fails the check rather than passing it, and a kernel without `CLONE_PIDFD` leaves the run unowned, so nothing inside it can sudo.
...
**The executor daemon is the one exception, closed differently.** It runs as the uid every brokered command runs as, sits in no run's cgroup, and receives each run's whole environment over its socket, so it is the single process from which every run's injected values could be read. What refuses a brokered command reaching *into* it is `PR_SET_DUMPABLE=0`, set by both daemons at startup: it refuses same-uid `ptrace` whatever `ptrace_scope` says (`0` on RHEL, Fedora and Arch). A seccomp filter is not the alternative on a host installed with `--allow-sudo`, and cannot be: refusing the syscall that way forces `NoNewPrivileges=` on, which makes `sudo` inert.

**The approved command itself persisting root: not closable.** It spends its legitimate root once and installs a setuid-root binary, a `systemd` unit, a `cron` entry or a line in `sudoers`, none of which involves faramir again or ends when the run does. Configuring a host and backdooring it are the same primitives, so no sandbox distinguishes them without an allowlist of exactly which files, which is not "run a playbook". Any credential would be defeated equally.
...
The uid stays the bound. `faramir-exec` gains the ability to *ask*, not the ability to sudo, and the ask is answered by an account the agent cannot become. What remains is operational: an operator with `NOPASSWD` sudo or a warm sudo timestamp has already handed the agent that account, and a watcher left in a terminal the agent can type into is a prompt the agent can answer. A passwordless sudoers grant for the executor's account, `NOPASSWD` or `Defaults !authenticate`, would remove all of this in one line, so `faramir doctor` checks that account for either spelling whether or not this host was installed with `--allow-sudo`.
