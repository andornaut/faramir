...
A home is not mounted until its owner logs in, so the secrets are absent at boot and to cron. Absent is therefore never quietly tolerated.

**One question for both.** A brokered command's output is redacted against the same value set, so what makes `redact` unsafe makes `exec` unsafe: output holding a managed secret the redactor does not have. The test is whether a managed file exists whose contents went unread, so at least one file matched and every matched file loaded. How many secrets came out of them does not enter into it, an empty file holding nothing to miss. Nothing configured and an entry that named none are refused alike, since a file may be absent because it was never written or because the filesystem holding it is not mounted, and only the second is dangerous. That costs a fresh install every wrapped command until it has one file, which is loud and says why. The daemon comes up regardless: `faramir status` and `faramir doctor` are what explain missing secrets, and both need the process they are asking. `--check` and `doctor` fail on all of them; the daemon refusing per request is what makes coming up safe.

Checked per request rather than once at startup, so a reload that shrinks the set is caught instead of leaving the broker serving a set it knows is short. The exception is a keeper that could not be reached once a set has loaded, where what is kept is the last thing known to be true, unconfirmed rather than short; a cold start has nothing to keep and refuses. Checking per request is what makes this placement survivable at all: `--check` runs from `init` and `doctor`, neither of which is around at boot, which is exactly when an unmounted home is absent. `init` also refuses to write into an unmounted encrypted home, which lands in the backing directory and is shadowed the moment the home mounts.
