...
The first is a *second, unapproved command* riding the approval. Every brokered command runs as `faramir-exec`; `/proc/<pid>/environ` is readable within a uid, so a concurrent run could read the approved run's token and `sudo` on it. Demonstrated on a live host before it was closed. The broker closes it by serialising: an elevation is approved only when its run is the sole brokered command in flight, and while the approval is live every other brokered command is refused `busy`. The two enforcement points are symmetric under one lock — registering a run blocks a new approval, a live approval blocks a new registration — so a live approval and any other registered run never coexist, and the window has no second process to do the stealing. It rests on no `faramir-exec` process outliving its run, and that is what the per-run cgroup buys: every brokered command — elevation or not — is spawned into a cgroup of its own (clone3's `CLONE_INTO_CGROUP`, the unit granted `Delegate=`), and the whole cgroup is killed and drained when the run ends, so a `setsid` child that breaks out of the process group is reaped with it. This is the **one** mechanism that ends a run's processes: there is no process-group kill beside it, because a process group is a strictly weaker grouping the same `setsid` escapes, and a straggler it let through is exactly the failure this closes. So there is nothing to fall back to — a host that cannot confine (no cgroup v2, no `Delegate=`, or a kernel below 5.14 without `cgroup.kill`) refuses the run rather than run it unreaped, because a silent fallback is the weaker-posture-that-still-looks-fine this whole exercise is against. Old kernels are not supported; they fail closed. Applying it to every run rather than only elevating ones is both simpler — one path, one reaper — and a hardening in its own right: an ordinary brokered command can no longer leak a process that holds the working tree or a slot open past its life. `faramir doctor` checks the delegation on every install so the refusal is never a mystery.
