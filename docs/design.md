...
**One question at a time, and never a queue.** Runs that all registered while the host was quiet can each reach a `sudo`; the first files the question and the rest are refused, naming the command that holds it. A queue there could only ever have held questions that cannot be answered yes: two questions mean two registered runs, and approving requires sole occupancy, so every queued question's outcomes were a refusal and an expiry, and what it added was a list of prompts for an operator to work through, none of them grantable. Attention is the scarce thing this design spends, and a queue spends it on nothing. Requests from the *same* run still join that run's own question, which is what makes one approval cover a playbook's twenty become'd tasks.
