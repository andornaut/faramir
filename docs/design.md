...
**Naming the command means rendering it, not printing it.** The argv is the caller's, it arrives as JSON strings, and the question is printed to a terminal, which obeys what it is sent. Most of that is already gone: everything handed to a question has been through the redactor, which strips CSI (colour included), OSC and the C0 controls. Two things survive it, and either is enough. A bare `\r` returns the cursor, so the rest of the line overwrites what came before it, only CRLF being normalised; and `ESC` followed by a byte outside `@-Z` and `\-_` matches no pattern there, which includes `ESC c`, a full terminal reset that on many emulators takes the scrollback with it. Left raw, a run could therefore overwrite the question it is being judged on. Every caller-chosen part of a question is quoted the moment it holds anything but printable text, ordinary arguments are left bare (a prompt full of quotation marks is one that is read less carefully), and the command is bounded, the audit record keeping the whole of it.

The same rendering covers `faramir logs`, which is the other place this text reaches a root terminal and the place the design sends an operator to see what an approved command did. [internal/termsafe](../internal/termsafe/termsafe.go) holds the rule for both. Recorded output is escaped rather than quoted and is never bounded, being the text an operator came to read; there is no formatting to lose, the redactor having taken the colour on the way in. The prompt also names what argv[0] *resolved to* when that is not what argv[0] says: a relative program resolves against the request's cwd, which is the agent's working tree, so `bin/ansible-playbook` can be a file the agent wrote.
...
- **Not systemd-ask-password either**, which is the tool this looks like it should use. Asking through it means creating a file in `/run/systemd/ask-password`, `0755 root:root`, so a broker running as its own uid is refused and every request would be denied for want of anybody to ask. What that channel offers is a reply socket only root can write to, and the approval commands make the same guarantee directly: `SO_PEERCRED` on the broker's own socket, uid 0 or nothing. A `notify_command` may still announce a question; it carries no answer.
...
The broker closes it by serialising: an approval is approved only when its run is the sole brokered command in flight, and while the approval is live every other brokered command is refused `approval_in_progress`. That is a terminal refusal rather than a `busy` to retry, because a caller retrying against a live approval is one polling the exact interval the serialisation exists to protect, landing the moment it ends. The two enforcement points are symmetric under one lock (registering a run blocks a new approval, a live approval blocks a new registration), so a live approval and any other registered run never coexist, and the window has no second process to do the stealing. A question merely *pending* holds a new command too, which is not that rule twice: without it, a caller free to keep starting commands is the one deciding whether the host is ever quiet enough for a yes to take, and the operator answers, is refused for want of quiescence, and answers again. The cost is that one unanswered question stalls unrelated brokered work for up to `[sudo] timeout_sec`, which is the cost an approved run already imposes for its whole length.

**No question that could only be refused.** Runs that all registered while the host was quiet can each reach a `sudo`, and none of them files a question while another run is registered: the `sudo` is refused there and then, naming the command in the way. A question filed in that state could only ever be answered no, approving requiring sole occupancy and a second registered run being what denies it, so what it would add is a prompt, a yes and a refusal. Attention is the scarce thing this design spends. One question at a time and never a queue follows rather than being a second rule: a question exists only where its run is the only one registered, and a pending question admits no new run beside it. Requests from the *same* run still join that run's own question, which is what makes one approval cover a playbook's twenty become'd tasks.
