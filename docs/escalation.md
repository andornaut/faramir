...
**Only what you type against the question counts.** Before each prompt `sudo watch` flushes the terminal's input queue, and it drops any line its reader had already taken off that queue before the question was shown. Keys pressed while nothing was pending never become an answer, however fast the next question arrives; type it again at the prompt.
