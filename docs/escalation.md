...
3. Your watcher prints it and reads your answer from **its** terminal:
   ...
   `received` | The wall clock `sudo` asked at, which puts the question beside everything else stamped in this terminal: a question raised a minute ago and one raised at lunchtime read alike when all that is printed is what remains of the timeout. Same shape as the day heading `faramir logs` prints, so the two clocks an operator reads are one clock
   `expires` | What is left of the timeout, counting down to the refusal. This is what an answer is typed against
   `waited` | How long the command has been blocked, from the moment `sudo` asked, shown wherever it rounds to a second or more. Past tense because the line is printed once and never rewritten: it is the wait as it stood when the question was printed, not a figure counting up. It is the command's wait rather than a report on whoever is answering, so a watcher running the whole time still shows a second or two for its own start, the password it was run under, or the poll round trip. Read it at the sizes that mean something
...
--- | --- | ---
`sudo credential` | `faramir-exec` holds no passwordless sudoers grant, in either spelling (`NOPASSWD` or `Defaults !authenticate`), and no password of its own: the two ways it could sudo with the broker out of the way. A `sudo -l` that fails to answer reports as unchecked rather than as no entry, a sudoers this sudo refuses to parse reading the same as one holding nothing | still checked
`sudo grant` | The PAM stack gates rather than falls open: `requisite`, `seteuid` and faramir's own helper, matched as fields rather than substrings, with nothing standing ahead of the helper to answer before the broker is asked (only the sudo-rs branch may). The helper and the environment file are unwritable by the executor and by you, `/etc/pam.d/other` is not a free pass, and the arrangement matches the sudo this host actually has: on sudo-rs the branch is still in both shared stacks, and on the original no branch is left over | `n/a`
