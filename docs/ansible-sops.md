...
`faramir init` mints one, `0600 faramir-broker`, beside the age key, and renders `[ssh] key` itself — a drop-in setting it is refused. Put the public half `init` prints into `authorized_keys` on each managed host; `--ssh-key` moves the key, or adopts one you placed yourself.
