...
The encrypted file belongs in the secrets directory, `/etc/faramir/secrets` unless `--config-dir` moved it. The operator is not in the group that owns it, so putting a file there and editing it afterwards both go through `sudo faramir vault edit`.
...
```bash
sudo faramir vault add ansible-ctrl
```
...
Every edit after that is `sudo faramir vault edit ansible-ctrl`, re-encrypting to the recipients the file already had.
