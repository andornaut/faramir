...
There is a ceiling too, and it is the kernel's rather than a setting: Linux caps one environment variable at 128 KiB, so a value at or over that could never be injected into a command. It is refused the same way. Holding one is not free either: every value enters an automaton whose states carry a dense transition table, so the set costs roughly 19 KB of memory per byte of secret, and a value that size would take the broker to several gigabytes and then to the OOM killer, at which point nothing is redacted at all.
