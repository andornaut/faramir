...
A streaming `faramir redact` sends a chunk when it has a chunk's worth or after a short idle: without the idle a backgrounded command that prints a line and then waits, a dev server's "listening on ..." banner, would hold that line until it produced a whole chunk or exited, which for a server is never. The idle chunk is still marked `more`, so the broker keeps holding the tail; only the last chunk at end of input releases it. What that costs is that output shorter than the tail (twice the longest variant, plus a margin) is not shown until more arrives or the input ends: the tail is exactly the bytes a value could still be split across, so releasing it early to show them is the one thing that would leave a split value uncovered.
