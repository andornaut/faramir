...
//
// One reader for every caller that asks.  `faramir recipient reseal` re-encrypts the store
...
// path_regex, so a pattern anchored on that word reads a two-rule file as one.
// It costs no new dependency: what is kept out of the shipped binary is the sops
// libraries, and a YAML parser is not one of them.
package sopsrule
...
type Rule struct {
	...
	// without repeats.
	Recipients []string
	// ShamirThreshold is how many key groups have to come together to open a
	// file, and zero where the rule does not split the data key.  Carried so a
	// caller can refuse a rule it would otherwise flatten.
	ShamirThreshold int
}
...
// Recipients is every recipient across every rule, without repeats.
//
// For the caller whose question is only whether a key is named in the file at
// all: re-implementing which rule governs which file would be a second answer
// free to disagree with sops'.
...
type rule struct {
	// Both spellings sops takes.  `age` here is the shorthand a hand-edited file
	// often carries, and only here does sops accept a comma-separated string for
	// it; the installer writes key_groups.
	Age             ageList    `yaml:"age"`
	KeyGroups       []keyGroup `yaml:"key_groups"`
	ShamirThreshold int        `yaml:"shamir_threshold"`
}

// keyGroup is one group of a rule's key groups.
//
// Merge is followed, because sops follows it: a group may pull in others, and
// the keys they name seal the file exactly like the ones written inline.  A
// reader that stopped at the top level would report a rule as sealing to fewer
// recipients than it does, and a caller re-encrypting from that answer drops
// every reader named only under a merge -- silently, and unrecoverably by
// re-running.
//
// Age is a plain list rather than the shorthand: sops takes a comma-separated
// string only in a rule's own `age`, so accepting one here would read a file
// sops refuses to load.
...
// recipients is who the rule actually seals to.
//
// The key groups alone where a rule carries both, because that is what sops
// does: it reads the `age` shorthand only when there are no key groups.  So a
// rule with `age: A` beside a group naming B seals to B and to nobody else, and
// reading both would name a reader the rule does not grant -- which re-encrypts
// the store to a key it never listed, and lets a check pass on a rule whose
// groups leave the keeper out because the shorthand still mentions it.
...
// of YAML on a loaded host, short enough that a rule naming a recipient nothing
// can reach costs a few seconds rather than a command that never returns: a
// plugin recipient (age1yubikey1...) execs its plugin, which can sit waiting for
// somebody to touch a key.
...
// is free to disagree with the first, which is how a store comes to be reported
// as covered right up until an edit fails on it.
//
...
// what is wanted is only whether sops found a rule.  recipients are named on the
// command line where the caller has them, so a rule whose own recipients are
// unusable still answers the question that was asked.
func Covers(sopsPath, configPath string, recipients []string, target string) (bool, error) {
	...
	// --config rather than the SOPS_CONFIG variable: the variable is the newer of
	// the two, and a sops old enough not to know it searches from the working
	// directory instead, which would have this answer about a different file.
...
// probeBody is a document sops can parse as the store the target's name selects.
//
// The name decides the store, and --filename-override is what this hands sops,
// so a YAML body under a .json or .env name is one sops rejects before it has
...
// lastLine is the most useful line of what a failed sops printed, or "".
