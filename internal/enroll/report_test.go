...
// manager reads instead of stat-ing the tree, and the fields it reads are named
...
// touching a tag. internal/install holds the same test for its own report.
func TestTheReportSerialisesTheFieldsItPromises(t *testing.T) {
	body, err := json.Marshal(Report{
		Version: "v", Dir: "/d", ClientGroup: "g",
		Steps: []steps.Step{{Name: "s"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, key := range []string{"version", "dir", "group", "changed", "steps"} {
		if _, ok := got[key]; !ok {
