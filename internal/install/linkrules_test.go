...
func TestALinkedPathIsRefusedToClaudeAndThePluginHosts(t *testing.T) {
	layout := testLayout()
	layout.LinkedPaths = []string{"/home/operator/.config/gh/hosts.yml"}
...
func TestLinkedPathsAreCleanedAndOrdered(t *testing.T) {
	got := linkedPaths(Layout{LinkedPaths: []string{"/b", "", "/a", "/b"}})
...
func TestNoLinksRendersNoSection(t *testing.T) {
	...
	if strings.Contains(string(body), "[[secrets.link]]") {
...
// The coupling this check exists for: links are the operator's, in config.d,
// and the rules are init's, so a link added since the last run is uncovered.
...
func TestALinkedPathIsQuotedIntoJSON(t *testing.T) {
	layout := testLayout()
	layout.LinkedPaths = []string{filepath.Join("/home/operator", `odd"name`, "token")}
