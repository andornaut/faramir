...
func SectionFile(f hostfs.FS, path, section, head string, uid, gid int, within string) (bool, error) {
	...
	// A link followed and the owner checked; see hostfs.FS.EditedFile.
...
func OutOfDate(err error) bool {
	return errors.Is(err, errHalfMarked) ||
		...
		errors.Is(err, hostfs.ErrNotOperators)
}
