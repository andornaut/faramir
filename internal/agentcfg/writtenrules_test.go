...
func TestAnUnreadableRecordReadsAsNothingWritten(t *testing.T) {
	...
	if _, err := (hostfs.FS{}).WriteFile(writtenRulesPath(dir), []byte("{not json"), 0o600, hostfs.Keep, hostfs.Keep); err != nil {
		t.Fatal(err)
	}
	if got := ReadWrittenRules(dir); len(got) != 0 {
...
func TestASecondRunRemovesTheRuleTheConfigStoppedDeclaring(t *testing.T) {
	...
	run := func(rules ...string) []string {
		t.Helper()
		render := func(File) ([]byte, error) { return settings(rules...), nil }
		if _, _, err := WriteFiles(
			hostfs.FS{}, nil, home, configDir, os.Getuid(), hostfs.Keep, 0o700, false, render, files); err != nil {
			t.Fatal(err)
		}
...
func TestRecordingRulesRefusesToClobberAnotherRun(t *testing.T) {
	...
	_, err = hostfs.FS{}.WriteFileExpecting(writtenRulesPath(dir), append(body, '\n'), 0o600, before)
	if err == nil {
