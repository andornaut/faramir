...
// registers.
func renderCodexHook(t *testing.T, file File) (codexHook, string) {
	t.Helper()
	body, err := AssetFor(Targets["codex"], file, t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	var doc codexHook
...
func TestTheTwoHalvesOfACodexEnrolmentDifferByTheApproval(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		if !strings.HasPrefix(tc.command, hostlayout.DefaultBinDir+"/faramir guard") {
...
func TestCodexWritesOnePathUnderTwoRoots(t *testing.T) {
	...
	for _, file := range []File{target.AccountFiles[0], target.Files[0]} {
		if !file.Merge {
...
func TestCodexSaysWhatItCannotCheck(t *testing.T) {
	...
	for _, name := range Known() {
		if name != "codex" && Targets[name].AccountNote != "" {
