...
// A rule reaches from a command to a path in that same command, and no further.
// The subject is one this fixture protects, or the case decides nothing: a name
// no rendered rule carries is allowed whatever the span does.
func TestARuleReachesToTheEndOfItsOwnCommand(t *testing.T) {
	read := compiled(t, Dir("/etc/faramir")).read
	// Out of reach: the reader belongs to an earlier command.
	for _, cmd := range []string{
		`head -20 README.md; echo "/etc/faramir is where it lives"`,
		`head -20 README.md && echo "/etc/faramir"`,
		`head -20 README.md & echo "/etc/faramir"`,
		"head -20 README.md\necho \"/etc/faramir\"",
	} {
		if read.MatchString(cmd) {
			t.Errorf("%q is refused: a reader in an earlier command reached a later one", cmd)
		}
	}
	// In reach: one command, however the separator characters appear inside it.
	for _, cmd := range []string{
		`cat /etc/faramir/age.key`,
		`head -20 README.md; cat /etc/faramir/age.key`,
		`head -20 README.md && cat /etc/faramir/age.key`,
		// A separator inside a quoted argument is an argument, not a separator.
		`python3 -c 'import os; print(open("/etc/faramir/age.key").read())'`,
		`awk 'BEGIN{a=1;b=2}{print}' /etc/faramir/age.key`,
		`tar cf - --exclude='a;b' /etc/faramir`,
		// The path inside the same quoted string as the separator, which is why
		// the closing quote is optional.
		`python3 -c "x=1; open('/etc/faramir/age.key')"`,
		// A quote that is never closed still leaves the rest of the command.
		`cat "unterminated /etc/faramir/age.key`,
		`cat 'unterminated /etc/faramir/age.key`,
		// "&" as redirection rather than as backgrounding.
		`cat 2>&1 /etc/faramir/age.key`,
		`cat &> /tmp/x /etc/faramir/age.key`,
		`cat 1>&2 /etc/faramir/age.key`,
		// One command written over two lines.
		"cat \\\n  /etc/faramir/age.key",
	} {
		if !read.MatchString(cmd) {
			t.Errorf("%q is allowed, and the reader reaches the path in its own command", cmd)
		}
	}
	// The write rule spans arguments the same way, and sed is a writer rather
	// than a reader, so its quoted script is checked against that one.
	write := compiled(t, Dir("/etc/faramir")).write
	for _, cmd := range []string{
		`sed -i 's/a/b/;s/c/d/' /etc/faramir/config.toml`,
		`chmod 0644 /etc/faramir/age.key`,
		`rm -rf --preserve-root='a;b' /etc/faramir`,
		`cp 2>&1 /tmp/x /etc/faramir/age.key`,
	} {
		if !write.MatchString(cmd) {
			t.Errorf("%q is allowed, and the writer reaches the path in its own command", cmd)
		}
	}
	if write.MatchString(`rm -rf build; echo "/etc/faramir"`) {
		t.Error("a writer in an earlier command reached a later one")
	}
}
