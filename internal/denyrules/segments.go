...
//
// A heredoc body is read as commands, quoted delimiter or not. A quoted body is
// literal data where it is written to a file, and it is not data where the
// heredoc feeds an interpreter: `bash <<'EOF'` runs every line of it, and so do
// `sh`, `python3` and a body piped into any of them. Nothing in the redirection
// says which of those it is, so the body that is a script and the body that is a
// document are the same bytes, and reading them as commands is the answer that
// refuses the script. What it costs is a refused write of a document that quotes
// an operator command, which is a refusal and not a disclosure.
//
// Where the line cannot be read the whole of it is returned as one segment. A
// longer segment matches everything a shorter one would and more, so an
// unterminated quote, or any shape not accounted for here, costs a refusal that
// need not have happened rather than letting one through.
