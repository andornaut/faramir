...
func TestTheQuestionPaintsItsLabelsAndNotItsValues(t *testing.T) {
	...
	out, _ := testio.CaptureStdout(t, func() int { PrintQuestion(question, termuitest.Always(t)); return 0 })
...
func TestTheQuestionCarriesNoEscapesWithColourOff(t *testing.T) {
	out, _ := testio.CaptureStdout(t, func() int { PrintQuestion(askedQuestion(), termuitest.Plain(t)); return 0 })
...
func TestTheEndingsCarryTheSameColoursAsTheLog(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			out, _ := testio.CaptureStdout(t, func() int { PrintOutcome(tc.outcome, termuitest.Always(t)); return 0 })
...
func TestOnlyACleanEndingIsGreen(t *testing.T) {
	out, _ := testio.CaptureStdout(t, func() int {
		PrintOutcome(escalation.Outcome{LogID: "log-2", ExitCode: new(2), DurationSec: 1.2}, termuitest.Always(t))
		return 0
	})
...
func TestTheAnswerPromptIsBold(t *testing.T) {
	...
	out, _ := testio.CaptureStdout(t, func() int {
		ReadLines(termuitest.Always(t)).Answer(time.Now().Add(time.Minute))
		return 0
	})
...
func TestTheQuestionReadsInTheOrderItIsJudgedIn(t *testing.T) {
	...
	out, _ := testio.CaptureStdout(t, func() int { PrintQuestion(question, termuitest.Plain(t)); return 0 })
