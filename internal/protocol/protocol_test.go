...
// The token is a readability affordance.  It becomes a shell *variable
// reference*, so the value still never appears in any argv.
func TestTokenBecomesAVariableReference(t *testing.T) {
	cmd, refs, err := ResolveInlineTokens(
		[]string{"bash", "-lc", "curl -u admin:{{SECRET:home/router/admin}} http://x"},
		map[string]string{})
	if err != nil {
		t.Fatal(err)
	}
	name := EnvNameForRef("home/router/admin")
	if !strings.Contains(cmd[2], "${"+name+"}") {
		t.Errorf("argv was not rewritten: %q", cmd[2])
	}
	if strings.Contains(cmd[2], "{{SECRET:") {
		t.Errorf("token survived: %q", cmd[2])
	}
	if refs[name] != "secret://home/router/admin" {
		t.Errorf("ref not bound: %v", refs)
	}
}

func TestExistingBindingIsReused(t *testing.T) {
	cmd, refs, err := ResolveInlineTokens(
		[]string{"bash", "-lc", "echo {{SECRET:home/router/admin}}"},
		map[string]string{"ROUTER_PW": "secret://home/router/admin"})
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(cmd[2], "${ROUTER_PW}") {
		t.Errorf("the caller's own name was not reused: %q", cmd[2])
	}
	if len(refs) != 1 {
		t.Errorf("a second binding was invented: %v", refs)
	}
}

func TestUntouchedWhenNoToken(t *testing.T) {
	in := []string{"printenv", "ROUTER_PW"}
	cmd, refs, err := ResolveInlineTokens(in, map[string]string{"ROUTER_PW": "secret://a/b"})
	if err != nil {
		t.Fatal(err)
	}
	if strings.Join(cmd, " ") != strings.Join(in, " ") {
		t.Errorf("argv changed: %v", cmd)
	}
	if len(refs) != 1 {
		t.Errorf("refs changed: %v", refs)
	}
}

func TestEnvNameIsDeterministic(t *testing.T) {
	cases := map[string]string{
		"home/router/admin": "FARAMIR_HOME_ROUTER_ADMIN",
		"a.b-c/d":           "FARAMIR_A_B_C_D",
		"x":                 "FARAMIR_X",
	}
	for ref, want := range cases {
		if got := EnvNameForRef(ref); got != want {
			t.Errorf("EnvNameForRef(%q) = %q, want %q", ref, got, want)
		}
	}
}

// A malformed URI in env_refs must surface as an error, not a silent skip.
func TestInlineResolutionRejectsABadURI(t *testing.T) {
	_, _, err := ResolveInlineTokens([]string{"ls"}, map[string]string{"X": "hunter2"})
	if err == nil {
		t.Fatal("a literal value in env_refs was accepted")
	}
}
