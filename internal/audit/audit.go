...
// two Logs in one process (a test, or `faramir recipient reseal` opening its own) must not
...
// is what keeps `faramir secrets edit` and `faramir recipient reseal`, which write their own, from
