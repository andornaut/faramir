...
func enrolTree(t *testing.T, configDir string, names ...string) string {
	...
	for _, name := range names {
		...
		render := func(file agentcfg.File) ([]byte, error) {
			return agentcfg.AssetFor(target, file, configDir)
		}
		if _, _, err := agentcfg.WriteFiles(
			hostfs.FS{}, nil, tree, "", hostfs.Keep, hostfs.Keep, 0o2770|os.ModeSetgid, true, render, target.Files); err != nil {
			t.Fatal(err)
		}
	}
	...
	section := agentcfg.SectionBegin + "\ntest section\n" + agentcfg.SectionEnd + "\n"
	if err := os.WriteFile(agentcfg.TreeInstructionsFile(tree), []byte(section), 0o640); err != nil {
		t.Fatal(err)
	}
	for _, name := range names {
		rules := agentcfg.Targets[name].TreeInstructions
		if rules.Path == "" {
			continue
		}
	...
	if err := agentcfg.RecordEnrolment(configDir, agentcfg.EnrolledTree{
...
func findings(report Report, name string) []Finding {
	var out []Finding
...
func TestTreeConfigAcceptsTheProjectsOwnKeysBesideOurs(t *testing.T) {
	...
	theirs, err := agentcfg.MergeJSON(current, []byte(`{"model":"opus","env":{"X":"1"}}`+"\n"), nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEditableFilesReportsWhatAnInstallWouldRefuse(t *testing.T) {
	...
	var report Report
	reportEditableFiles(&report, home, os.Getuid(), Options{ConfigDir: t.TempDir()})
...
func TestEditableFilesReportsATreesOwnInstructionsFile(t *testing.T) {
	...
	var report Report
	reportEditableFiles(&report, t.TempDir(), os.Getuid(), Options{ConfigDir: configDir})
...
func TestEditableFilesIsOKWhereEveryFileIsTheOperatorsOwn(t *testing.T) {
	var report Report
	reportEditableFiles(&report, t.TempDir(), os.Getuid(), Options{ConfigDir: t.TempDir()})
...
func TestEditableFilesIsOKWhereThereIsNothingToRefuse(t *testing.T) {
	var report Report
	diagnoseEditableFiles(&report, Options{ConfigDir: t.TempDir()})
...
func TestTreeModesReportsWritableFilesAndUnstickyDirectories(t *testing.T) {
	...
	var report Report
	diagnoseTreeModes(&report, Options{ConfigDir: configDir})
...
func TestWhatAnEnrolmentWritesIsWhatDoctorCompares(t *testing.T) {
	...
	// not.
	merged, err := agentcfg.MergeJSON(nil, body, nil)
	if err != nil {
		t.Fatal(err)
	}
