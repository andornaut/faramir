...
func TestRuleEntriesReadsBothShapes(t *testing.T) {
	got, err := agentcfg.RuleEntries([]byte(`{
...
func TestAStaleRuleIsFound(t *testing.T) {
	...
	got, err := agentcfg.StaleRules(filepath.Join(home, ".claude/settings.json"), current, "/etc/faramir")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAFreshlyWrittenFileHasNoStaleRules(t *testing.T) {
	...
	got, err := agentcfg.StaleRules(path, current, "/etc/faramir")
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 0 {
...
func TestLooksManagedMatchesOnlyTheInstallersOwnLine(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.entry, func(t *testing.T) {
			if got := agentcfg.LooksManaged(tc.entry, configDir); got != tc.want {
