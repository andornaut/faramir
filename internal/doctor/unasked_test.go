...
func TestTheSSHKeyCheckDoesNotPassOnAnUnaskedOperator(t *testing.T) {
	...
	var unnamed Report
	diagnoseSSHKey(&unnamed, Options{ExecUser: "ex"}, cfg)
	...
	var named Report
	diagnoseSSHKey(&named, Options{ExecUser: "ex", AgentUser: "op"}, cfg)
...
func TestTheSudoArrangementDoesNotPassOnAnUnaskedOperator(t *testing.T) {
	...
	var unnamed Report
	diagnoseSudoArrangement(&unnamed, Options{ExecUser: "ex"}, cfg)
	finding := only(t, unnamed)
	if finding.Status != StatusWarn {
		t.Errorf("status %q, want %q: %s", finding.Status, StatusWarn, finding.Detail)
	}
	...
	var named Report
	diagnoseSudoArrangement(&named, Options{ExecUser: "ex", AgentUser: "op"}, cfg)
	if got := only(t, named); got.Status != StatusOK {
		t.Errorf("status %q, want %q: %s", got.Status, StatusOK, got.Detail)
	}
	if named.NotAsked != 0 {
...
func TestTheGroupBailOutCountsEveryGroupItSkipped(t *testing.T) {
	var shared Report
	diagnoseGroup(&shared, Options{ClientGroup: "dev", SecretsGroup: "dev"})
	if shared.NotAsked != 1 {
		t.Errorf("counted %d unasked with one group, want 1", shared.NotAsked)
	}

	var split Report
	diagnoseGroup(&split, Options{ClientGroup: "dev", SecretsGroup: "faramir-keeper"})
	if split.NotAsked != 2 {
...
func TestLogRotationIsReportedWithoutRoot(t *testing.T) {
	...
	dir := t.TempDir()
	original := hostunit.SystemUnitDir
	hostunit.SystemUnitDir = dir
	t.Cleanup(func() { hostunit.SystemUnitDir = original })
...
func TestEveryWarnThatSaysItDidNotAskCountsItself(t *testing.T) {
	...
	for _, probe := range []struct {
		...
		{"no systemd", func(r *Report) {
			original := hostunit.Running
			hostunit.Running = func() bool { return false }
			defer func() { hostunit.Running = original }()
			diagnoseUnits(r, Options{})
		}},
		{"no broker", func(r *Report) { diagnoseVersion(r, Options{}) }},
		{"no operator", func(r *Report) { diagnoseOperatorKeys(r, Options{}) }},
		{"no root", func(r *Report) {
			if os.Geteuid() == 0 {
				return // the branch under test is the one a caller without root takes
			}
			diagnoseBoundaries(r, Options{}, &config.Config{}, servesUnknown)
		}},
	} {
