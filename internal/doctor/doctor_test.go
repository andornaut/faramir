...
func onlyFinding(t *testing.T, report Report, name string) Finding {
	t.Helper()
	var found []Finding
...
func TestDiagnoseSopsRecipients(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			want: StatusWarn, says: []string{"none of which", "cannot decrypt", "reader add"},
		},
		{
...
func TestOnlyNotRedactableSeparatesAValueFromAFault(t *testing.T) {
	loaded := func() brokercheck.CheckReport {
	...
	for _, tc := range []struct {
		name   string
		report func() brokercheck.CheckReport
		want   bool
	}{
		{"a short ref and nothing else", loaded, true},
		{"no short refs at all", func() brokercheck.CheckReport {
			r := loaded()
			r.Secrets.NotRedactable = nil
			return r
		}, false},
		{"a socket policy problem beside it", func() brokercheck.CheckReport {
			r := loaded()
			r.Policy = []string{"[keeper] allowed_user names the wrong account"}
			return r
		}, false},
		{"a file that did not load beside it", func() brokercheck.CheckReport {
			r := loaded()
		...
		{"an entry that named no file beside it", func() brokercheck.CheckReport {
			r := loaded()
			r.Secrets.UnresolvedPatterns = []string{"/etc/faramir/secrets/*.sops.yml"}
			return r
		}, true},
		{"nothing loaded at all", func() brokercheck.CheckReport {
			r := loaded()
			r.Secrets.Count = 0
			return r
		}, true},
		{"a shadowed ref beside it", func() brokercheck.CheckReport {
			r := loaded()
...
func TestOnlyDegradedLinksSeparatesALinkFromAFault(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"no degraded links at all", func() brokercheck.CheckReport {
			r := loaded()
		...
		{"a managed file that did not load beside it", func() brokercheck.CheckReport {
			r := loaded()
			r.Secrets.Errors = []string{"app.sops.yml: bad mac"}
			return r
		}, false},
		{"a ref too short to cover beside it", func() brokercheck.CheckReport {
			r := loaded()
			r.Secrets.NotRedactable = map[string]string{"short/pin": "shorter than 8 characters"}
			return r
		}, false},
...
func TestUnitsReportTheStateSampledBeforeTheBrokerWasAsked(t *testing.T) {
	original := hostunit.Running
	hostunit.Running = func() bool { return true }
	defer func() { hostunit.Running = original }()

	var report Report
	diagnoseUnits(&report, Options{SocketStates: map[string]string{
...
func TestVersionSkewIsAFindingOfItsOwn(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			var report Report
			diagnoseVersion(&report, Options{BrokerVersion: tc.running})
...
func TestWhatSkipsTheSSHAgentProbe(t *testing.T) {
	...
	var noKey Report
	diagnoseSSHAgent(&noKey, Options{}, &config.Config{}, servesUnknown)
	...
	var noConfig Report
	diagnoseSSHAgent(&noConfig, Options{}, nil, servesUnknown)
...
func TestTheBrokeredCommandIsSkippedWhenItCannotBeSent(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			var report Report
			diagnoseBrokered(&report, Options{BrokerVersion: tc.version}, &config.Config{}, tc.serves)
...
func stubLogrotate(t *testing.T, rule, state string) (rulePath, statePath string) {
	...
	originalRule, originalState := hostlayout.LogrotateConfig, hostlayout.LogrotateStatePaths
	hostlayout.LogrotateConfig, hostlayout.LogrotateStatePaths = rulePath, []string{statePath}
	t.Cleanup(func() { hostlayout.LogrotateConfig, hostlayout.LogrotateStatePaths = originalRule, originalState })
...
func TestTheVersionCheckSeesTwoBuildsOfOneVersion(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			var report Report
			diagnoseVersion(&report, Options{
...
func TestDoctorRefusesAnAccountThatIsNotThere(t *testing.T) {
	...
	report := Diagnose(Options{ConfigDir: dir, AgentUser: "nosuchuser-uxqz"})
	if !report.Failed {
	...
	if quiet := Diagnose(Options{ConfigDir: dir}); len(quiet.Findings) < 2 {
...
func TestWhichCheckFailuresAnInstallFinishesOver(t *testing.T) {
	for _, tc := range []struct {
		...
		{"a link whose file has gone", func() brokercheck.CheckReport {
			var r brokercheck.CheckReport
			r.Secrets.Count = 3
			r.Secrets.DegradedLinks = map[string]string{"npm/token": "not there"}
			return r
		}, true},
		{"a value too short to redact", func() brokercheck.CheckReport {
			var r brokercheck.CheckReport
			r.Secrets.Count = 3
			r.Secrets.NotRedactable = map[string]string{"short/pin": "shorter than 8"}
			return r
		}, true},
		{"a first install with no secrets written yet", func() brokercheck.CheckReport {
			var r brokercheck.CheckReport
			r.Secrets.Patterns = []string{"/etc/faramir/secrets/*.sops.yml"}
			r.Secrets.UnresolvedPatterns = []string{"/etc/faramir/secrets/*.sops.yml"}
			return r
		}, true},
		{"a managed file that did not load", func() brokercheck.CheckReport {
			var r brokercheck.CheckReport
			r.Secrets.Files = []string{"app.sops.yml"}
			r.Secrets.Errors = []string{"app.sops.yml: bad mac"}
			return r
		}, false},
		{"a socket policy problem", func() brokercheck.CheckReport {
			var r brokercheck.CheckReport
			r.Policy = []string{"[keeper] allowed_user names the wrong account"}
			return r
		}, false},
