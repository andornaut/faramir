#!/usr/bin/env bash
# Remove the broker.  Leaves accounts, the config, the store and the audit log
# alone -- deleting the age key would make every sops file in the repo
...
Left in place on purpose:
  ...
  ${SECRETS_DIR}/ -- the managed sops files
  ${CONFIG_DIR}/config.toml -- the base config
  ${CONFIG_DIR}/config.d/ -- per-consumer settings merged over it
